...
/**
 * Main function that matches Python API
...
function imageTemplate({
  ...
  const cloudinaryUrlBase = 'https://res.cloudinary.com/canonical/image/fetch';
  const bypassExtensions = ['.svg', '.webp', '.avif', '.gif'];
  ...
  const shouldBypass = bypassExtensions.some(ext => 
  ...
  const imageSrc = `${cloudinaryUrlBase}/${stdDefCloudinaryAttrs}/${encodedUrl}`;
  ...
  if (widthInt > 100) {
    ...
    const createSrcsetUrl = (width, options) => {
      ...
      return `${cloudinaryUrlBase}/${widthAttrs}/${encodedUrl} ${width}w`;
    };
